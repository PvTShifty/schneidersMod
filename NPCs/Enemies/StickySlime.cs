        ...
        public override void HitEffect(int hitDirection, double damage) {
			if (npc.life > 0) {
				for (int i = 0; i < damage / npc.lifeMax * 100; i++) {
					Dust dust = Dust.NewDustDirect(npc.position, npc.width/5, npc.height/5, 182, hitDirection, -1f, 100);
					dust.noGravity = false;
				}
				return;
			}
			for (int i = 0; i < 50; i++) {
				Dust dust = Dust.NewDustDirect(npc.position, npc.width/5, npc.height/5, 182, 2 * hitDirection, -2f, 100);
				dust.noGravity = false;
			}
		}
    }
